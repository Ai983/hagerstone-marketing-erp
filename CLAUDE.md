...
**Feedback loop** (migration `022`): (1) `document_performance()` — per document: shares, leads, leads moved to a later stage after the first share (read from `stage_change` interactions), leads won after it and won value; shown as "What's working" + a line per card on `/documents`. (2) `leads.win_trigger / win_why_us / win_worry` — optional "why they bought" answers in the Won step (`WinStoryFields`, helpers in `lib/utils/win-story.ts`) and editable later in the drawer (`WinStoryCard`). (3) `/feedback` page: objections (30d), loss reasons + competitors (90d), win stories, the AI review (`/api/ai/feedback-review` — GET latest, POST generates from 90 days of losses/objections/win stories, refuses with 422 when there are fewer than 3 signals, saves to `marketing.ai_reviews`, which only the service role writes), and the `marketing.feedback_actions` board (heard → meaning → action → owner sales/marketing/founder → due; AI suggestions add with one click).
